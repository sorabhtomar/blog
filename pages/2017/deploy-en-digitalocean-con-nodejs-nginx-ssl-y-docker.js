...
export default withViews(({ views }) => (
  <Post views={views}>
    ...
    <Snippet>
      <Comment>{'# creamos un volumen para almacenar los certificados'}</Comment>
      <Comment>{'# para luego compartirlos entre contenedores'}</Comment>
    ...
    <Snippet>
      ...
      {'crontab -l > tempcron'}
      <Comment>{'# agregamos el comando docker run'}</Comment>
      <Comment>{'# con certbot para que se ejecute a las 1:00 los lunes'}</Comment>
    ...
    <P>Para este ejemplo usaremos mi <A url='https://github.com/jlobos/blog'>blog</A>, ya que está en un repositorio listo con su Dockerfile.</P>
    ...
    <Snippet>
      <Comment>{'# creamos la imagen'}</Comment>
      {'docker build -t blog https://github.com/jlobos/blog.git'}
      <Comment>{'# corremos el container'}</Comment>
      {'docker run -d --name blog blog'}
    ...
    <Snippet>
      ...
      <Comment>{'# corremos el container de nginx, '}</Comment>
      <Comment>{'# lo enlazamos con nuestra app (blog)'}</Comment>
      <Comment>{'# y le indicamos los puertos'}</Comment>
