...
export default () =>
  <App>
    ...
    <Article>
      ...
      <section>
        <P>
          En este artículo explicaré como montar una arquitectura de principio a
          fin con Node.js, Nginx como proxy y un certificado gratis SSL con
          Certbot todo esto manejando contenedores de Docker.
        </P>
        <Quote>
          Todo este procedimiento se realizó para levantar este blog, en la
          lectura aparecerán los diferentes repositorios.
        </Quote>
        <P>
        ...
        <Quote>
          ...
          consigas USD $10 gratis, empezarás tu aventura con USD $15 que serian
          3 meses del servicio básico.
        </Quote>
        <Quote>
          Comenzamos creamos un nuevo Droplet de Digitalocean, escogemos Ubuntu
          como distribución con la versión por defecto, el tamaño más pequeñito
          (será suficiente), seleccionamos la región que más nos acomode y
...
cat ~/.ssh/id_rsa.pub`}</Snippet>
        ...
        <P>
          Ya dentro de nuestra máquina en la nube, instalaremos Docker. Una
          forma rápida y fácil para tener la versión más reciente, es mediante:
        </P>
...
docker version`}</Snippet>
        ...
        <P>
          Crearemos un certificado ssl (https) gratis y automáticamente gracias
          a{' '}
...
docker volume create --name nginx-certs`}</Snippet>
        <P>
          Ahora crearemos un contenedor con la{' '}
          <a href={links[3]}>imagen oficial</a> de <Code>cerbot</Code>, este se
          conectara entre nuestra máquina y el proveedor del certificado para
          eso le indicamos los puertos 80 y 443 (no debe haber nada ocupando
          esos puertos), y guardará los certificados en el volumen de docker que
          acabamos de crear (<Code>-v nginx-certs:/etc/letsencrypt</Code>).
        </P>
        <P>
        ...
        <P>
          Si quieres una versión interactiva de <Code>certbot</Code> (te pedirá
        ...
        <P>
          Hay que tener en cuenta que el certificado por seguridad expira cada
          90 días, para validarlo solo debemos hacer:
        </P>
        <Snippet
...
crontab -l`}</Snippet>
        ...
        <P>
          Para este ejemplo usaremos mi <a href={links[4]}>blog</a>, que ya está
          en una imagen pública en <a href={links[5]}>Docker Hub</a>.
        </P>
        <Quote>
...
docker ps`}</Snippet>
        ...
        <Quote>
          ...
          certificado ‘Diffie-Hellman’ de 2048 bits, para hacer más segura
          nuestra configuración SSL.
        </Quote>
        <Quote>
          Es importante adecuar la configuración de nginx a tu proyecto, puedes
          ver que existen parámetros que se deben cambiar (<a href={links[9]}>
...
docker run --name nginx -v nginx-certs:/etc/nginx/certs:ro -p 80:80 -p 443:443 -d --link blog blog-nginx`}</Snippet>
        <P>
          ...
          contenedores Docker y una app escuchando en el puerto 80, ahora
          podemos <a href={links[11]}>comprobar nuestro certificado</a> y
          obtener un A+.
        </P>
      </section>
      <footer>
